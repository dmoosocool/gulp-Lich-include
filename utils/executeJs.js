...
module.exports = function executeJs(file, type, ctx) {
    ...
    files.forEach(function (item) {
        ...
        if ('npm' === type) {
            // 将npm文件写入到输出目录中.
            let npmFile = path.join(process.cwd(), 'node_modules', item) + _this.config.jsExtension,
                distFile = path.join(_this.config.distDir, 'node_modules', item) + _this.config.jsExtension;
